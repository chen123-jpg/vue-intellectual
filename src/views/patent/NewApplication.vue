<template>
  <div class="page">
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.isEdit ? '编辑新申请' : '新增新申请'" width="700px" destroy-on-close>
...
const handleDelete = async (id) => {
  try {
    ...
    const res = await moduleApi.remove(id)
...
const handleBatchDelete = async () => {
  try {
    ...
    const res = await moduleApi.batchRemove(selected.value.map(r => r.id))
