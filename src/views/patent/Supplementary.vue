<template>
  <div class="page">
    <el-card>
      ...
      <div class="toolbar">
        <el-button v-if="hasPerm('patent:supplementary:add')" type="primary" @click="openAdd">新增</el-button>
        <el-button v-if="hasPerm('patent:supplementary:delete')" type="danger" :disabled="!selected.length" @click="handleBatchDelete">批量删除</el-button>
      </div>
      ...
      <el-table :data="tableData" v-loading="loading" border stripe @selection-change="onSelectionChange">
        ...
        <el-table-column prop="applicationDate" label="申请日" width="110" />
        <el-table-column prop="feeReduction" label="费减" width="80" />
        <el-table-column label="操作" width="180" fixed="right">
          <template #default="{ row }">
            <el-button v-if="hasPerm('patent:supplementary:edit')" size="small" type="primary" @click="openEdit(row)">编辑</el-button>
            <el-button v-if="hasPerm('patent:supplementary:delete')" size="small" type="danger" @click="handleDelete(row.id)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.isEdit ? '编辑补漏' : '新增补漏'" width="700px" destroy-on-close>
      <el-form :model="form" label-width="100px">
        <el-row :gutter="20">
          ...
          <el-col :span="12"><el-form-item label="费减"><el-input v-model="form.feeReduction" /></el-form-item></el-col>
...
const moduleApi = api['supplementary']
...
const form = reactive({
  ...
  sponsor: '', agent: '', applicationDate: '', feeReduction: '', remark: ''
})
