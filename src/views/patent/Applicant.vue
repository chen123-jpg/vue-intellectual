  ...
  <div class="page">
    <el-card>
      ...
      <div class="toolbar">
        ...
        <el-button v-if="hasPerm('patent:applicant:delete')" type="danger" :disabled="!selected.length" @click="handleBatchDelete">
          批量删除
        </el-button>
      </div>
      ...
      <el-table :data="tableData" v-loading="loading" border stripe @selection-change="onSelectionChange">
        ...
        <el-table-column label="操作" width="180" fixed="right">
          <template #default="{ row }">
            <el-button v-if="hasPerm('patent:applicant:edit')" size="small" type="primary" @click="openEdit(row)">编辑</el-button>
            <el-button v-if="hasPerm('patent:applicant:delete')" size="small" type="danger" @click="handleDelete(row.id)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.isEdit ? '编辑申请人' : '新增申请人'" width="500px" destroy-on-close>
...
import { getApplicantList, getApplicantById, createApplicant, updateApplicant, removeApplicant, batchRemoveApplicant } from '../../api/externalPersonnelController.js'
...
const openEdit = async (row) => {
  try {
    const res = await getApplicantById(row.id)
    if (res.code === 200) {
...
const handleSave = async () => {
  if (!form.name) { ElMessage.warning('请输入申请人姓名'); return }
  saving.value = true
  try {
    const res = dialog.isEdit ? await updateApplicant({ ...form }) : await createApplicant({ ...form })
    if (res.code === 200) {
...
const handleDelete = async (id) => {
  try {
    ...
    const res = await removeApplicant(id)
...
const handleBatchDelete = async () => {
  try {
    ...
    const res = await batchRemoveApplicant(selected.value.map(r => r.id))
